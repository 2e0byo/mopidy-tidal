...
from mopidy_tidal.lru_cache import id_to_cachef
...
def _parse_size(s: str) -> int:
    ...
    match = search(r"([0-9]+\.*[0-9]*) *([kmgt]*)", s.lower())
    if not match:
        raise ValueError(f"Unable to parse size: {s}")
...
class CachingRetriever:
    ...
    def _background_retrieve(self, url: str, outf: Path):
        ...
        with Path("/tmp/queue").open("a") as f:
            f.write("done\n")
    ...
    def get_cached(self, key: str) -> str:
        """Get a cached file."""
        outf = self._cache_dir / id_to_cachef(key)
...
class TidalPlaybackProvider(backend.PlaybackProvider):
    def __init__(self, audio: Any, backend: backend.Backend) -> None:
    ...
    def translate_uri(self, uri):
        ...
        if self._cache_enabled:
            try:
                ...
                remote_url = session.track(track_id).get_url()
                final_url = self.caching_retriever.get(uri, remote_url)
        else:
            final_url = session.track(track_id).get_url()
